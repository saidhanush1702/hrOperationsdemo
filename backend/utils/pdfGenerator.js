...
export const buildCombinedPDFBuffer = async (invoiceId) => {
    ...
    //
    //   A. generated by this app (lives in INVOICE_DIR) — the invoice page only,
    ...
    if (invoiceData.invoice_file_path) {
        try {
            const storedBytes = await fs.readFile(invoiceData.invoice_file_path);
            combinedPdf = await PDFDocument.load(storedBytes, { ignoreEncryption: true });

            const storedAbs = path.resolve(invoiceData.invoice_file_path);
            const isAppGenerated = storedAbs.startsWith(path.resolve(INVOICE_DIR) + path.sep);
            appendTimesheets = isAppGenerated; // case B keeps its own bundled scans
        } catch (e) {
    ...
    // the path just written. Doing the save here rather than in the callers means
    ...
    if (!combinedPdf) {
        try {
            ...
            combinedPdf      = await PDFDocument.load(await fs.readFile(savedPath));
    ...
    // Last resort: the save failed (read-only disk, permissions, full volume). Build
    // the page in memory so downloads and emails still work, just without persisting.
    if (!combinedPdf) {
        let orgLogoBase64 = null;
        if (invoiceData.org_logo_url) {
            try {
                const logoPath = path.join(__dirname, '../', invoiceData.org_logo_url.replace(/^\//, ''));
                const logoBuffer = await fs.readFile(logoPath);
                const ext  = path.extname(invoiceData.org_logo_url).toLowerCase().replace('.', '');
                const mime = (ext === 'jpg' || ext === 'jpeg') ? 'image/jpeg' : 'image/png';
                orgLogoBase64 = `data:${mime};base64,${logoBuffer.toString('base64')}`;
            } catch (e) { console.warn('Could not load org logo for PDF, skipping:', e.message); }
        }
    ...
    try {
        ...
        for (const ts of timesheets) {
            try {
                let rawPath = ts.attachment_url;
                if (rawPath.startsWith('/')) rawPath = rawPath.substring(1);
                const absoluteTsPath = path.join(__dirname, '../', rawPath);
                const fileBytes = await fs.readFile(absoluteTsPath);
                const ext = path.extname(absoluteTsPath).toLowerCase();
...
export const generateAndSaveInvoicePDF = async (invoiceId) => {
    ...
    const fileName = `${invoiceData.invoice_number}_${Date.now()}.pdf`;
    const filePath = path.join(INVOICE_DIR, fileName);
    await fs.writeFile(filePath, invoiceBuffer);

    const previousPath = invoiceData.invoice_file_path;
    await pool.query(`UPDATE invoices SET invoice_file_path = ? WHERE id = ?`, [filePath, invoiceId]);

    // The filename carries a timestamp, so every regeneration writes a new file and
    // would otherwise orphan the last one on disk. Remove the superseded copy once
    // the DB points at the replacement, so a failure here can never leave the row
    // referencing a file that is already gone.
    //
    // Only ever delete our own output: old-portal imports (blob/..., 2022/...) live
    // outside INVOICE_DIR and are the sole copy of those documents.
    if (previousPath && previousPath !== filePath) {
        const prevAbs = path.resolve(previousPath);
        if (prevAbs.startsWith(path.resolve(INVOICE_DIR) + path.sep)) {
            try { await fs.unlink(prevAbs); }
            catch (e) { if (e.code !== 'ENOENT') console.warn('Could not remove superseded invoice PDF:', e.message); }
        }
    }
